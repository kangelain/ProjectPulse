        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-sm">
            {[
              ...
              { icon: Users, label: "Team & Stakeholders", content: `Lead: ${project.teamLead} | Portfolio: ${project.portfolio}` },
          ...
          {project.keyMilestones.length > 0 ? (
            <ul className="space-y-4">
              {project.keyMilestones.map((milestone) => {
                ...
                return (
                  <li key={milestone.id} className="flex items-center space-x-4 p-4 border rounded-lg shadow-sm hover:shadow-md transition-shadow bg-secondary/20">
                     <Badge
                      className={cn(
